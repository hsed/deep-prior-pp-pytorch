...
def main():    
    ...
    transform_pca = PCATransform(n_components=PCA_COMP, use_cache=True, overwrite_cache=False)

    transform_train = DeepPriorXYTransform(depthmap_px=IMGSZ_PX, crop_len_mm=CROPSZ_MM)
    # transform_val = DeepPriorXYTransform() # its equivalent as train transformer no need to do extra
    ...
    if transform_pca.transform_matrix_np is None:
        ...
        #         break
        #print(fullList)
        print("\n\nSAMPLES SHAPE: ", y_train_samples.shape, 
    ...
    print("FINAL_AVG_3D_ERROR_MM: ", test_res_collector.calc_avg_3D_error())
